...
export const RIDDLE_BALANCE_ENDPOINT_PATH = "/v1/balance" as const;
...
export async function getRiddleBalance(config: RiddleClientConfig = {}): Promise<RiddleBalanceResult> {
  return riddleRequestJson<RiddleBalanceResult>(config, RIDDLE_BALANCE_ENDPOINT_PATH, { method: "GET" });
}
...
function riddleJobArtifactsEndpointPath(jobId: string): string {
  if (!jobId?.trim()) throw new Error("jobId is required");
  return `/v1/jobs/${jobId}/artifacts`;
}

export function getRiddleJobArtifacts(
  config: RiddleClientConfig,
  jobId: string,
): Promise<Record<string, unknown>> {
  return riddleRequestJson<Record<string, unknown>>(
    config,
    riddleJobArtifactsEndpointPath(jobId),
  );
}
...
export async function pollRiddleJob(
  ...
  const artifacts = await getRiddleJobArtifacts(config, jobId);
  return {
...
export function createRiddleApiClient(config: RiddleClientConfig = {}) {
  return {
    ...
    getJobArtifacts: (jobId: string) =>
      getRiddleJobArtifacts(config, jobId),
