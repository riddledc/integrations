...
- 4cb748c: Add reusable audio mix intent route-alignment receipts so app proof loops can keep current-target routes consistent with selected claim-candidate intents without app-specific glue.
...
- 5c65d7e: Add Neon Run 011 post-deploy batch evidence showing fast mix health, mobile layout, playback sync, bounded deep exploration, and durable current-target proof after the guitar 0.55 override promotion.
