...
- 523a00e: Add independently pinned rule trust roots and reusable evidence-template trust roots, bounded exact observation schemas, single-artifact capture identity enforcement, nested checked-meaning composition, replay-derived evidence linkage, generic content-free receipts bound to private payload bytes, deterministic replay and chronology checks, approved execution-policy binding, and core-owned pure profile evaluators while keeping client semantics, network, and browser operations outside core. The compatibility facade now keeps core external instead of rebundling a second implementation copy.
- Updated dependencies [523a00e]
...
- f9d2730: Fix PR comment rendering for hosted profile result packets.
...
- abe9722: Expose public-state consumer surface summaries for hosted proof views and agent summaries, and pin checkpoint hard-blocker rejection for optimistic ready-to-ship checkpoint responses.
...
- 2ed5870: Report semantic winner recovery evidence as restart/reset visibility when the receipt is backed by New Game, New Round, New Route, restart, or reset controls instead of a Play Again button.
