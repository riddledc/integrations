...
1. Capture an invoice, purchase order, and receipt as one immutable record set.
2. Recompute line extensions, subtotals, stated tax arithmetic, and totals in
   integer minor units.
3. Check exact identities, currency, terms, line items, prices, quantities, and
   totals across the three records.
4. If the one supported over-invoicing case is present, create a new invoice
   revision through an exact app-owned correction.
5. Check the new record set afresh, reusing only the unchanged purchase-order
   and receipt proof branches.
...
2. Select **Create corrected invoice revision**. The app writes
   `invoice.r2.json` without overwriting revision 1. The new revision has no
   inherited result.
3. Check the new record set. All 11 requirements pass. The UI identifies the
   exact PO and receipt certificates that were reused and the invoice-dependent
   branches that were checked again.
...
The hostile suite includes wrong SKU with unchanged totals, wrong currency,
partial receipt, unsafe integers, record-role substitution, changed policy,
stale files, raw-byte/signature/nonce tampering, an old closure offered for a
new subject, caller-supplied correction fields, symlinked fixtures/workspaces,
and pre-existing output preservation.

## Quiet proof plumbing

The ordinary interface shows documents, calculations, mismatches, the exact
bounded correction, immutable history, and reused versus recomputed branches.
...
nonce IDs, signed bundle IDs, and deterministic replay result. It contains no
filenames, workstation paths, document content, raw nonce, raw signature,
...
and it contains only the synthetic input copies and app-created invoice
revision.
...
- the selected structured JSON bytes were captured under the pinned synthetic
  policy;
...
This first client accepts only the three explicit synthetic JSON schemas.
Arbitrary PDF, DOCX, XLSX, email, OCR, partial-receipt policy, tolerances,
credits, and payment records are intentionally excluded.
...
Lean does not parse JSON or prove the runtime's safe-integer and 1,000-line
bounds, canonical encoding, hashing, signatures, file currentness, source
authenticity, or the truth of facts outside the captured records. Those remain
explicit runtime and trust-boundary obligations.
