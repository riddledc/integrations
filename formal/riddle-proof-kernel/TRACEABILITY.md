...
| `accepted_response_has_matching_advertised_packet` | `formal-conformance.test.js` checkpoint decision enum/allowed decision alignment checks | active checkpoint responses must match identity, resume token, and advertised decisions |
...
| `checkpoint_lifecycle_summary_projects_state`, `accepted_advancing_response_clears_pending_packet`, `accepted_blocking_response_retains_pending_packet`, `rejected_response_preserves_lifecycle_state`, and `duplicate_response_does_not_increment_response_count` | `formal-conformance.test.js` checkpoint lifecycle summary cases; `proof-run.test.js` blocked duplicate checkpoint test | checkpoint summary must distinguish pending packets, accepted responses, rejected blockers, and duplicate responses |
| `clearing_blocking_response_loses_pending_packet` and `counting_rejected_response_inflates_accepted_count` | `formal-conformance.test.js` blocking/rejected summary cases | blocking responses retain pending packets; rejected responses must not become accepted response history |
