...
| Public-state consumer surfaces | `packages/riddle-proof/src/public-state.ts`; proof-run result JSON | PR comments, hosted proof views, agent wrappers, status monitors | `packages/riddle-proof/src/pr-comment.ts`; `packages/riddle-proof/test.js`; `packages/riddle-proof/formal-conformance.test.js` | `PublicConsumerSurface`, `publicConsumerSurfaceConforms`, `public_consumer_surface_from_state_conforms`, `stale_merge_recommendation_consumer_violates_held_public_state`, `missing_checkpoint_audit_consumer_violates_public_state` | Semantic obligation: downstream surfaces must be generated from public-state booleans/disclosures and must not reintroduce prohibited success, merge, sync, ship, or checkpoint-completeness claims. |
...
| `public_consumer_surface_from_state_conforms`, `stale_merge_recommendation_consumer_violates_held_public_state`, and `missing_checkpoint_audit_consumer_violates_public_state` | `formal-conformance.test.js` PR-comment consumer cases; `test.js` held, handoff-ready, and blocked PR comment cases | public consumers must derive handoff/status claims from public state and suppress stale merge recommendations under prohibited claims |
